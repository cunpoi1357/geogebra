...
function MultipleChoiceQuestion({ data }) {
    ...
    const sortFn = item =>
        Number(toArray(item.question.match(/^Câu (\d+)\./))[1])
    ...
    const lengthMaxOfQuestions = max([
        data.A.length,
        data.B.length,
        data.C.length,
        data.D.length,
    ...
    return (
        <div className='relative flex flex-col h-[calc(100vh-96px)] overflow-hidden'>
            <header className='py-3 px-4 w-[100wh] max-h-[50%] lg:max-h-max overflow-auto lg:overflow-visible bg-[#f0f9fe] text-xl m-1 border border-[#00adf1] shadow-[#9897ff] shadow-lg'>
                ...
                <Latex>
                    {data.question.split(/^Câu (\d+)\./)[2] || data.question}
                </Latex>
            </header>
            <section className='overflow-auto md:p-10'>
                ...
                <div className='flex flex-col items-center p-4'>
                    ...
                    <div className='md:w-[800px] flex w-full mt-8 justify-between'>
                        <Link
                            to={
                                listId[currentIndex - 1]
                                    ? `/example/${listId[currentIndex - 1]}`
                                    : ''
                            }
                        >
